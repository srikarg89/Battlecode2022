...
public class Miner extends Robot {
    ...
    HashSet<Integer> forgottenMinesSet = new HashSet<Integer>();
    ...
    public void run() throws GameActionException {
        ...
        int numForgotten = 0;
        MapLocation[] updatedForgotten = new MapLocation[forgottenMines.length];
        for(int i = 0; i < forgottenMines.length; i++){
            if(forgottenMines[i] == null){
                continue;
            }
            if(myLoc.distanceSquaredTo(forgottenMines[i]) <= myType.visionRadiusSquared){
                updatedForgotten[numForgotten] = forgottenMines[i];
                numForgotten++;
            }
            else{
                forgottenMinesSet.remove(Util.mapLocationToInt(forgottenMines[i]));
            }
        }
        forgottenMines = updatedForgotten;
        forgottenMinesIdx = numForgotten;

        // System.out.println("After forgetting: " + Clock.getBytecodesLeft());

        indicatorString += "C " + rc.getMovementCooldownTurns() + ", " + rc.getActionCooldownTurns() + "; ";
        runMovement(nearbyDangerousEnemies, numDangerousEnemies);
        // System.out.println("After Movement: " + Clock.getBytecodesLeft());
        tryMineAllDirections(); // 700 bytecode
        checkPossibleDeath();
    }

    public boolean runMovement(RobotInfo[] nearbyDangerousEnemies, int numDangerousEnemies) throws GameActionException {
        ...
        fillAdjTeammate();
        // System.out.println("After filling variables: " + Clock.getBytecodesLeft()); // 4600
        double currHeuristic = getHeursitic(myLoc, nearbyEnemies); // Costs 500 bytecode
        double bestHeuristic = currHeuristic;
        // System.out.println("Getting heuristic: " + Clock.getBytecodesLeft());
        // System.out.println("My location: " + myLoc.toString());
        if(currHeuristic < 0){
            // System.out.println("Path A");
            MapLocation enemyCOM = Util.calculateEnemySoldierCOM(nearbyDangerousEnemies);
            Direction retreatDir = myLoc.directionTo(enemyCOM).opposite();
            return nav.goTo(myLoc.add(retreatDir).add(retreatDir).add(retreatDir).add(retreatDir).add(retreatDir).add(retreatDir));
        }
            ...
            if (bestDir != null) { // If you can move in a better direction, do so
                rc.move(bestDir);
                indicatorString += "BH: " + (int) (bestHeuristic * 100) + ", Dir: " + bestDir + "; ";
                return true;
            } else {
            ...
            MapLocation[] nearbyLeadSense2 = rc.senseNearbyLocationsWithLead(myType.visionRadiusSquared, 2);
//            MapLocation closestUnfarmedLead = getClosestUnfarmedLead(nearbyLeadSense2);
            MapLocation closestUnfarmedLead = getClosestNonforgottenLead(nearbyLeadSense2);
            MapLocation targetLoc = closestUnfarmedLead;
            // System.out.println("Unfarmed lead: " + Clock.getBytecodesLeft());
            if(targetLoc == null){
                targetLoc = findClosestLeadMine(ignore_farmer_threshold);
                // System.out.println("Ignore farmer threshold: " + Clock.getBytecodesLeft());
            }
            if (targetLoc == null) {
...
//                    nav.doingWeirdMinerPathing = true;
                    ...
                    targetLoc = myLoc;
                    for(int i = 5; i-- > 0; ){
                        if(!Util.onTheMap(targetLoc.add(towardsCurrentTarget))){
                            break;
                        }
...
//        double val = leadMineable - numTeammates - numEnemies * 6;
    ...
    // Find the closest location with the most reserves (gold and lead) to mine from. If there is no location to mine from, return null
    public MapLocation getClosestNonforgottenLead(MapLocation[] checkLocs) throws GameActionException {
        MapLocation bestLoc = null;
        int bestHeuristic = 100000;
        int dist; int cooldown; int heuristic;
        // Go to nearest lead mine
        for(int i = checkLocs.length; i -- > 0; ){
//            boolean valid = true;
//            for(int j = 0; j < forgottenMinesIdx; j++){
//                if(forgottenMines[j].equals(checkLocs[i])){
//                    valid = false;
//                    break;
//                }
//            }
...
//                continue;
//            }
            if(forgottenMinesSet.contains(Util.mapLocationToInt(checkLocs[i]))){
                continue;
            }
            dist = myLoc.distanceSquaredTo(checkLocs[i]);
            cooldown = 10 + rc.senseRubble(checkLocs[i]);
            heuristic = dist * cooldown;
            if(heuristic >= bestHeuristic){
                continue;
            }
    ...
    public void tryMineAllDirections() throws GameActionException {
        ...
        for(int i = 0; i < Util.allDirections.length; i++){
            ...
            if(mined){
                indicatorString += "L;";
//                boolean alrForgot = false;
//                for(int j = 0; j < forgottenMinesIdx; j++){
//                    if(loc.equals(forgottenMines[j])){
//                        alrForgot = true;
//                    }
//                }
                int locInt = Util.mapLocationToInt(loc);
                boolean alrForgot = forgottenMinesSet.contains(locInt);
                if(!alrForgot){
                    forgottenMines[forgottenMinesIdx % forgottenMines.length] = loc;
                    forgottenMinesIdx++;
                    forgottenMinesSet.add(locInt);
                }
            }
        }
    }
