...
//        Logger.Log("lead amount: " + rc.getTeamLeadAmount(this.myTeam));
        ...
        if(rc.getHealth() == myType.getMaxHealth(rc.getLevel()) && needHealing){
            ...
            comms.writeSharedArray(comms.ARCHON_HEALING_START_IDX + bestArchonForHealingIdx, rc.readSharedArray(comms.ARCHON_HEALING_START_IDX + bestArchonForHealingIdx) - 1);
            bestArchonForHealingIdx = -1;
        }
...
//        needHealing = false;
        if(needHealing){
            ...
            if(!enemyCOM.equals(myLoc)) {
                nav.goTo(myLoc.subtract(myLoc.directionTo(enemyCOM)).subtract(myLoc.directionTo(enemyCOM)).subtract(myLoc.directionTo(enemyCOM)).subtract(myLoc.directionTo(enemyCOM)).subtract(myLoc.directionTo(enemyCOM)).subtract(myLoc.directionTo(enemyCOM)));
            }
            else if (myLoc.distanceSquaredTo(targetLoc) > RobotType.ARCHON.actionRadiusSquared){
                nav.goTo(targetLoc);
            }
            else{
