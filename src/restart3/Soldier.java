...
//            }
    ...
    public boolean checkSafe(RobotInfo[] nearbyFriendlies, RobotInfo[] dangeorusEnemies, RobotInfo nearestEnemyInfo) throws GameActionException { // TODO: Maybe only check # of attackers on the robot closest to you?
        ...
        MapLocation nearestFriendlyArchon = Util.intToMapLocation(rc.readSharedArray(comms.getClosestFriendlyArchonIndex()));
        MapLocation nearestEnemyArchon = comms.getClosestEnemyArchonOnComms();
...
//            archonDistanceRatio = (double)myLoc.distanceSquaredTo(nearestEnemyArchon) / (double)myLoc.distanceSquaredTo(nearestFriendlyArchon);
            MapLocation enemyCOM = Util.calculateEnemySoldierCOM(nearbyEnemies);
            archonDistanceRatio = (double)enemyCOM.distanceSquaredTo(nearestFriendlyArchon)/(double)enemyCOM.distanceSquaredTo(nearestEnemyArchon) ;
        }

        double myTurnsNeeded = enemyHP / friendlyDamage;
        double enemyTurnsNeeded = friendlyHP / enemyDamage;

//        myTurnsNeeded *= archonDistanceRatio*archonDistanceRatio*archonDistanceRatio*archonDistanceRatio*archonDistanceRatio*archonDistanceRatio*archonDistanceRatio;
        myTurnsNeeded *= archonDistanceRatio*archonDistanceRatio;

//        System.out.println("Friendly HP: " + friendlyHP + ", DMG: " + friendlyDamage + ", Enemy HP: " + enemyHP + ", DMG: " + enemyDamage);

        indicatorString += "R:" + archonDistanceRatio + ",MT:" + (int)myTurnsNeeded + ",ET:" + (int)enemyTurnsNeeded + "; ";
